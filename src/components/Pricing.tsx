        ...
        <div className="grid lg:grid-cols-3 gap-0 border border-edge">
          {tiers.map((tier, i) => (
            ...
            >
              ...
              <button className={`w-full py-5 text-[11px] font-black uppercase tracking-[0.2em] transition-all group border ${tier.highlight ? 'bg-brand text-black border-brand hover:brightness-110' : 'bg-transparent text-white border-edge hover:border-brand hover:text-brand'}`}>
                {tier.cta}
              </button>
            </motion.div>
          ))}
